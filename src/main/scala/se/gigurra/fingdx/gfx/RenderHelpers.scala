...
trait RenderHelpers { _: RenderContext.type =>
  ...
  def make11OrthoCamera(scale: Float = 1.0f): Unit = {
    ...
    if (w > h) {
      gdxCamera.viewportWidth = scale * 2.0f * w / h
      gdxCamera.viewportHeight = scale * 2.0f
    } else {
      gdxCamera.viewportHeight = scale * 2.0f * h / w
      gdxCamera.viewportWidth = scale * 2.0f
    }

    gdxCamera.position.set(0.0f, 0.0f, 0.0f)
    gdxCamera.update()
    batch.setProjectionMatrix(gdxCamera.combined)
    shapeRenderer.setProjectionMatrix(gdxCamera.combined)
  }
  ...
  def scissor(at: Vec2 = Vec2(0,0), size: Vec2 = Vec2(2,2))(f: => Unit): Unit = {
    ...
    ScissorStack.calculateScissors(gdxCamera, transform.current, rIn, rOut)
...
trait Projection[+T] {
  ...
  def renderCenter: RenderCenter
}

case class ScreenProjection(renderCenter: RenderCenter) extends Projection[Any] {
...
case class World2DProjection(renderCenter: RenderCenter) extends Projection[Any] {
  ...
  def at(position: Vec3, heading: Double = 0.0)(f: => Unit): Unit = {
    transform(_
      .translate(position - renderCenter.position : Vec2)
      .rotate(-heading)) {
      f
    }
  }
