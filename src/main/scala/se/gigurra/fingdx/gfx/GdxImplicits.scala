...
trait GdxImplicits { _: RenderContext.type =>
  ...
  implicit class DrawableString(text: String) {
    ...
    def drawCentered[_: Projection](color: Color = null, scale: Float = 1.0f)(implicit configuration: GfxConfig): Unit = {
      transform(_
    ...
    def drawRightOf[_: Projection](color: Color = null, scale: Float = 1.0f)(implicit configuration: GfxConfig): Unit = {
      transform(_
        .scalexy(scale * 0.05f * symbolScaleF / font.size)
        .rotate(projection.headingCorrection)) {
    ...
    def drawLeftOf[_: Projection](color: Color = null, scale: Float = 1.0f)(implicit configuration: GfxConfig): Unit = {
      transform(_
        .scalexy(scale * 0.05f * symbolScaleF / font.size)
        .rotate(projection.headingCorrection)) {
        drawRaw(xAlign = -0.5f, yAlign = extraDownOffset, xRawOffs = -0.025f * symbolScaleF, color = color)
    ...
    def drawBelow[_: Projection](color: Color = null, scale: Float = 1.0f)(implicit configuration: GfxConfig): Unit = {
      transform(_
        .scalexy(scale * 0.05f * symbolScaleF / font.size)
        .rotate(projection.headingCorrection)) {
